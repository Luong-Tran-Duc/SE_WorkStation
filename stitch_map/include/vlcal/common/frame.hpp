...
public:
  ...
  bool has_colors() const;       ///< Check if the frame has point colors
  ...
  bool check_colors() const;       ///< Warn if the frame doesn't have colors
...
public:
  ...
  Eigen::Vector4d * colors;  ///< Point colors
...
struct traits<Frame> {
  ...
  static bool has_colors(const Frame& frame) { return frame.has_colors(); }
  ...
  static const  Eigen::Vector4d& colors(const Frame& frame, size_t i) { return frame.colors[i]; }
