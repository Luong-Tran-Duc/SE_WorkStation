...
FrameCPU::FrameCPU(const Frame& frame) {
  ...
  if (frame.colors) {
    add_colors(frame.colors, frame.size());
  }
...
// add_colors
template <typename T, int D>
void FrameCPU::add_colors(const Eigen::Matrix<T, D, 1>* colors, int num_points) {
  assert(num_points == size());
...
template void FrameCPU::add_colors(const Eigen::Matrix<float, 4, 1>* colors, int num_points);
template void FrameCPU::add_colors(const Eigen::Matrix<double, 4, 1>* colors, int num_points);
...
FrameCPU::Ptr voxelgrid_sampling(const Frame::ConstPtr& frame, const double voxel_resolution) {
  ...
  if (frame->colors) {
    ...
    std::transform(voxels.begin(), voxels.end(), downsampled->colors, [&](const Indices& indices) {
      Eigen::Vector4d sum = Eigen::Vector4d::Zero();
      for (const auto i : *indices) {
        sum += frame->colors[i];
      }
